...
func magicmouseBattery() -> (c: String, s: String, a: String) {
    let commandString = "ioreg -c BNBMouseDevice | grep BatteryPercent | sed 's/[a-z,A-Z, ,|,\",=]//g' | tail -1 | awk '{print $1}'"
    let percentString = shell(args: commandString)
    let (colorString, dropdownString, ansiColor) = calculateColorStrings(percentString: percentString)
    return (colorString, dropdownString, ansiColor)
}
...
(c,s,a) = magicmouseBattery()
powerStatus.trackpad.colorString = c
powerStatus.trackpad.dropdownString = s
powerStatus.trackpad.ansiColorString = a

// print("B: \u{001B}\(powerStatus.battery.ansiColorString)●\u{001B}[0m K: \u{001B}\(powerStatus.keyboard.ansiColorString)●\u{001B}[0m M: \u{001B}\(powerStatus.trackpad.ansiColorString)●\u{001B}[0m| size=10 ansi=true")
print("\u{001B}[37m B: \u{001B}\(powerStatus.battery.ansiColorString)●\u{001B}[37m M: \u{001B}\(powerStatus.trackpad.ansiColorString)●\u{001B}[0m| size=10 ansi=true")
print("---")
...
print("Mouse:".padding(length: 11) + powerStatus.trackpad.dropdownString + "| font=Courier color=\(powerStatus.trackpad.colorString)")
